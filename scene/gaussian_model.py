...
class GaussianModel:
    ...
    def compute_3D_filter(self, cameras):
        ...
        filter_3D = distance / focal_length * (0.2 ** 0.5)  # 得到采样间隔T^
    ...
    def remove_outliers_dbscan(self, xyz, eps=0.3, min_samples=60):
        ...
        参数：
        - xyz: 输入的点云数据，格式为 N x 3 的 NumPy 数组
        ...
        scaled_points = scaler.fit_transform(xyz)
        ...
        # 选取非离群点
        # filtered_points = xyz[np.any([labels == max_two_label_ids[0], labels == max_two_label_ids[1]], axis=0)]
    ...
    def save_filter_ply2(self, path, mip):
        ...
        p2_labels, p2_max_two_label_ids = self.remove_outliers_dbscan(p2, eps=0.3, min_samples=80)
        # p3 = p2[np.any([p2_labels == p2_max_two_label_ids[0], p2_labels == p2_max_two_label_ids[1]], axis=0)]
        p3 = p2[np.any([p2_labels == p2_max_two_label_ids[0]], axis=0)]
        
        # TODO 3、计算p3的aabb盒
        point_minx,point_maxx = np.min(p3[...,0]),np.max(p3[...,0])
        point_miny,point_maxy = np.min(p3[...,1]),np.max(p3[...,1])
        point_minz,point_maxz = np.min(p3[...,2]),np.max(p3[...,2])
        ...
        p4 = []
        labels = np.zeros(p1.shape[0])
        indexs = []
        for i, point in enumerate(p1):
            x, y, z = point
            if aabb[0] <= x <= aabb[3] and aabb[1] <= y <= aabb[4] and aabb[2] <= z <= aabb[5]:
                p4.append(point)
                labels[i] = 1
                indexs.append(i)
        ...
        # TODO 5、对p4聚类
        p4_labels, p4_max_two_label_ids = self.remove_outliers_dbscan(p4, eps=0.2, min_samples=60)
        # p4_filter = p4[p4_labels!=-1]
        # indexs = np.array(indexs)[p4_labels!=-1]
        indexs = np.array(indexs)[np.any([p4_labels == p4_max_two_label_ids[0]], axis=0)]
        
        # 对Gaussian的其他属性进行过滤
                
        filtered_xyz = p1[indexs]
        
        # if len(p4_filter) == len(filtered_xyz):
        #     print("chenggongle...")
        # Apply clustering to other attributes based on the filtered indices
        filtered_normals = np.zeros_like(filtered_xyz)
        filtered_f_dc = self._features_dc.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()[indexs]
        filtered_f_rest = self._features_rest.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()[indexs]
        filtered_opacities = self._opacity.detach().cpu().numpy()[indexs]
        filtered_scale = self._scaling.detach().cpu().numpy()[indexs]
        filtered_rotation = self._rotation.detach().cpu().numpy()[indexs]

        if mip:
            filtered_filter_3D = self.filter_3D.detach().cpu().numpy()[indexs]
            dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes(exclude_filter=False)]
            attributes = np.concatenate((filtered_xyz, filtered_normals, filtered_f_dc, filtered_f_rest, filtered_opacities, filtered_scale, filtered_rotation, filtered_filter_3D), axis=1)
        else:
            dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes(exclude_filter=True)]
